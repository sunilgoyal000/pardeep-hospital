"use client";
import {
  Users, Activity, CalendarDays, TrendingUp, TrendingDown, MoreHorizontal,
} from "lucide-react";
import type { DashboardOverview } from "@/modules/dashboard/schema";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs" style={{ color: "var(--admin-muted)" }}>vs prior period</span>
      </div>
    </div>
  );
}

function BarChart({ data }: { data: DashboardOverview["weeklyTrend"] }) {
  const max = Math.max(1, ...data.map((d) => d.patients), ...data.map((d) => d.appointments));
  const H = 120;
  const BAR_W = 28;
  const GAP = 14;
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.date}>
              ...
              <rect x={x} y={H - barH} width={BAR_W} height={barH} rx={6} fill="url(#barGrad)" />
              <rect
                x={x + 6}
              ...
              />
              <text x={x + BAR_W / 2} y={H + 18} textAnchor="middle" fontSize="11" fill="#94a3b8" fontFamily="Inter, sans-serif" fontWeight="500">
                {d.day}
              </text>
              <text x={x + BAR_W / 2} y={H - barH - 5} textAnchor="middle" fontSize="10" fill="#0d9488" fontFamily="Inter, sans-serif" fontWeight="600">
                {d.patients}
              </text>
            </g>
          );
...
const activityColors: Record<string, string> = {
  new: "#0891b2", active: "#22c55e", done: "#94a3b8", cancelled: "#ef4444", warning: "#f59e0b",
};
const activityIcons: Record<string, string> = {
  appointment: "📅", patient: "🧑", doctor: "👨‍⚕️", system: "⚙️",
};
...
export default function DashboardClient({ overview }: { overview: DashboardOverview }) {
  const { kpis, weeklyTrend, deptBreakdown, recentActivity, todaysAppointments, weekTotal } = overview;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "var(--admin-muted)" }}>
            {todayLabel} — Live hospital operations
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Total Patients" value={kpis.totalPatients} growth={kpis.patientGrowth} icon={Users} iconBg="#ccfbf1" iconColor="#0d9488" />
        <KPICard label="Active Queues" value={kpis.activeQueues} growth={kpis.queueGrowth} icon={Activity} iconBg="#cffafe" iconColor="#0891b2" />
        <KPICard label="Appointments Today" value={kpis.appointmentsToday} growth={kpis.appointmentGrowth} icon={CalendarDays} iconBg="#ede9fe" iconColor="#8b5cf6" />
        <KPICard label="Revenue Today" value={kpis.revenueToday} growth={kpis.revenueGrowth} icon={TrendingUp} iconBg="#fef3c7" iconColor="#d97706" prefix="₹" />
      </div>

      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-2xl p-5" style={{ background: "var(--admin-card)", border: "1px solid var(--admin-border)" }}>
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="font-bold text-base" style={{ color: "var(--admin-text)" }}>Weekly Patient Trend</h2>
              <p className="text-xs mt-0.5" style={{ color: "var(--admin-muted)" }}>Patients vs Appointments — last 7 days</p>
            </div>
        ...
        <div className="rounded-2xl p-5" style={{ background: "var(--admin-card)", border: "1px solid var(--admin-border)" }}>
          <div className="flex items-center justify-between mb-5">
          ...
          <div className="space-y-4">
            {deptBreakdown.length > 0 ? (
              deptBreakdown.map((d) => <DeptBar key={d.id} {...d} />)
            ) : (
              <p className="text-xs" style={{ color: "var(--admin-muted)" }}>No appointments this week.</p>
            )}
          </div>
          <div className="mt-4 pt-4 border-t" style={{ borderColor: "var(--admin-border)" }}>
            <p className="text-xs text-center" style={{ color: "var(--admin-muted)" }}>
              Total: <strong style={{ color: "var(--admin-text)" }}>{weekTotal}</strong> appointments this week
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-2xl overflow-hidden" style={{ background: "var(--admin-card)", border: "1px solid var(--admin-border)" }}>
          <div className="px-5 py-4 flex items-center justify-between border-b" style={{ borderColor: "var(--admin-border)" }}>
            <h2 className="font-bold text-base" style={{ color: "var(--admin-text)" }}>Recent Activity</h2>
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              <span className="text-xs" style={{ color: "var(--admin-muted)" }}>From audit log</span>
            </div>
          </div>
              ...
              recentActivity.map((a) => (
                <div key={a.id} className="flex items-start gap-3 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "var(--admin-muted)" }}>{relativeTime(a.createdAt)}</p>
                  </div>
                  <div className="w-2 h-2 rounded-full mt-2 flex-shrink-0" style={{ background: activityColors[a.status] || "#94a3b8" }} />
                </div>
              ))
            ) : (
              <div className="px-5 py-10 text-center text-sm" style={{ color: "var(--admin-muted)" }}>
                No recent activity yet. Book or cancel an appointment to see audit entries here.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: "var(--admin-card)", border: "1px solid var(--admin-border)" }}>
          <div className="px-5 py-4 border-b" style={{ borderColor: "var(--admin-border)" }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--admin-muted)" }}>{todayLabel}</p>
          </div>
          <div className="divide-y" style={{ borderColor: "var(--admin-border)" }}>
