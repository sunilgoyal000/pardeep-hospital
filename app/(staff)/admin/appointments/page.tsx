...
import {
  Plus, Search, Filter, Calendar, List, ChevronLeft, ChevronRight,
  Edit2, Trash2, Eye, Clock, User, Stethoscope,
} from "lucide-react";
...
export default function AppointmentsPage() {
  ...
  const apptDays = new Set(appointments.map((a) => parseInt(a.date.split("-")[2])));
        ...
        >
          ...
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: days }, (_, i) => i + 1).map((day) => {
              ...
              const isToday = day === 24 && calMonth === 3;
              return (
                <div
                  key={day}
                  ...
                  return (
                    <tr key={apt.id} className="group hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-4">
                        <span className="text-xs font-mono font-semibold" style={{ color: "#0d9488" }}>{apt.id}</span>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <div
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => showToast(`Viewing appointment ${apt.id}`)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-teal-50">
                            <Eye className="w-3.5 h-3.5 text-teal-600" />
                          </button>
                          <button onClick={() => showToast(`Editing appointment ${apt.id}`, "info")} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-blue-50">
                            <Edit2 className="w-3.5 h-3.5 text-blue-500" />
                          </button>
                          <button onClick={() => showToast(`Appointment ${apt.id} cancelled`, "error")} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-red-50">
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-lg rounded-2xl p-6 shadow-2xl" style={{ background: "var(--admin-card)" }}>
              <h2 className="font-bold text-lg mb-5" style={{ color: "var(--admin-text)" }}>Book New Appointment</h2>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: "Patient Name / ID", placeholder: "Search patient...", col: 2 },
                  { label: "Doctor", placeholder: "Select doctor", col: 1 },
                  { label: "Department", placeholder: "Select dept", col: 1 },
                  { label: "Date", placeholder: "YYYY-MM-DD", col: 1 },
                  { label: "Time Slot", placeholder: "e.g. 10:30 AM", col: 1 },
                  { label: "Notes (optional)", placeholder: "Chief complaint...", col: 2 },
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  onClick={() => { setShowModal(false); showToast("Appointment booked successfully!", "success"); }}
